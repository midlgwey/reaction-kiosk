...
export const createSuggestion = async (req, res) => {
  ...
  try {
    ...
    // Guardamos la sugerencia en la base de datos con un sentimiento inicial de "Pending"
    // (antes era "Neutral", pero eso hacía indistinguible un comentario ya analizado
    // de uno que falló silenciosamente en el análisis de fondo)
    const result = await db.execute({
      sql: `INSERT INTO suggestions (comment, rating_context, shift, sentiment, waiter_id, table_number) 
            VALUES (?, ?, ?, ?, ?, ?)`,
      args: [comment.trim(), rating_context || null, shift, 'Pending', waiter_id, table_number],
    });
...
// Diccionario de incidencias operativas críticas (red de seguridad, corre SIEMPRE,
// sin depender de que Gemini responda)
const criticalKeywords = [
  'mal','grosero','mal trato','de malas','sin ganas','actitud','prepotente','ignoraron','pesimo servicio','mala atencion','desatencion','mal servicio',
...
// Intenta parsear el JSON que devuelve Gemini, tolerando texto extra o fences de markdown
function parseGeminiJson(rawText) {
  try {
    const cleaned = rawText.replace(/```json|```/g, "").trim();
    const match = cleaned.match(/\{[\s\S]*\}/); // por si el modelo agrega texto antes/después
    ...
    const sentiment = ["Positive", "Negative", "Neutral"].includes(parsed.sentiment)
      ? parsed.sentiment
      : "Neutral";
    const hasComplaint = parsed.hasComplaint === true;

    return { sentiment, hasComplaint, ok: true };
  } catch (e) {
    return { sentiment: "Neutral", hasComplaint: false, ok: false };
  }
}
...
{"sentiment": "Positive" | "Negative" | "Neutral", "hasComplaint": true | false}

"hasComplaint" debe ser true si el comentario incluye CUALQUIER crítica, queja o punto negativo, incluso si el resto del comentario es positivo (ejemplo: "todo rico pero el café estaba frío" -> hasComplaint: true).

Comentario: "${commentText}"`;

    let aiSentiment = "Neutral";
    let aiHasComplaint = false;
    let aiOk = false;
    ...
    try {
      ...
      aiHasComplaint = parsed.hasComplaint;
      aiOk = parsed.ok;

      console.log(`[ID: ${id}] Gemini: sentiment=${aiSentiment} hasComplaint=${aiHasComplaint} parseOk=${aiOk}`);
    } catch (aiError) {
      // Cuota agotada, rate limit, timeout, etc. No relanzamos: seguimos con el diccionario.
    ...
    // Última red de seguridad: si algo truena de forma inesperada (ej. la query
    // a la BD), al menos dejamos constancia y evitamos que quede "Pending" para siempre.
    console.error(`[ID: ${id}] Error en proceso de fondo IA:`, dbError);
    try {
      await db.execute({
        sql: `UPDATE suggestions SET sentiment = 'Neutral' WHERE id = ? AND sentiment = 'Pending'`,
        args: [BigInt(id)]
      });
    } catch (fallbackError) {
      console.error(`[ID: ${id}] No se pudo aplicar fallback a Neutral:`, fallbackError);
    }
  }
}
      ...
      commentsList.forEach(item => {
        ...
        if (mode === 'bad') {
          ...
          const badWords = {
            ...
            'salado': 'Muy Salado', 'insipido': 'Sin Sabor', 'seco': 'Reseco'
          };
          ...
          const goodWords = {
            ...
            'amable': 'Amable', 'rapido': 'Rápido', 'perfecto': 'Perfecto', 'limpio': 'Limpio', 'deliciosa': 'Deliciosa',
